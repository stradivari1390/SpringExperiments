...
public class BookstoreUserRegister {
    ...
    public void registerNewUser(RegistrationForm registrationForm) {
        ...
        if (userEntityRepository.findByEmailOrPhoneNumber(registrationForm.getEmail()) == null) {
            UserEntity user = new UserEntity();
            user.setName(registrationForm.getName());
            user.setUsername(registrationForm.getEmail().substring(0, registrationForm.getEmail().indexOf("@")));
            user.setPassword(passwordEncoder.encode(registrationForm.getPass()));
            user.setBalance(0D);
            user.setRegTime(LocalDateTime.now());
            user.setHash(String.valueOf(user.getName().hashCode() + user.getRegTime().hashCode()));
            userEntityRepository.save(user);

            List<UserContactEntity> contacts =
                    List.of(new UserContactEntity(user.getId(), ContactType.EMAIL, (short) 0, registrationForm.getEmail()),
                    new UserContactEntity(user.getId(), ContactType.PHONE, (short) 0, registrationForm.getPhone()));
